<script lang="ts">
  ...
  const updateBoard = async (
    ...
    if (board) {
      ...
      if (!isEqual(props, state.props)) {
        changes.push({ type: 'set-props', props: props });
      }
      if (!isEqual(pieceDefs, state.pieceDefs)) {
        changes.push({ type: 'set-piece-defs', pieceDefs: pieceDefs });
      }
      if (changes.length > 0) {
  ...
  const archiveBoard = (type: BoardType) => {
    if (type == BoardType.archived) store.boardList.archiveBoard(boardHash);
    else if (type == BoardType.deleted) store.boardList.deleteBoard(boardHash);
