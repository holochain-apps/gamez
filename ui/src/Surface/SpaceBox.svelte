<script lang="ts">
  ...
  export let extraRotation: number = 0; // Additional rotation to apply
  let klass: string = '';
...
{#if scale}
  ...
  >
    ...
    height: ${box.h}px;
    transform: translate(${box.x}px, ${box.y}px) rotate(${(box.r || 0) + extraRotation}deg);`}
    >
      <slot />
    </div>
  </div>
{:else}
  <div
    ...
    transform: translate(${(box.x + $vp.panX) * $vp.zoom}px, ${(box.y + $vp.panY) * $vp.zoom}px) rotate(${box.r + extraRotation}deg);
    z-index: ${z * 10};`}
  >
    <slot />
  </div>
