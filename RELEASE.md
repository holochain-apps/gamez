...
Gamez is distributed as a `.webhapp` referenced by the
...
and 0.6 and 0.7 agents form disjoint networks. Game spaces created on the 0.6 line
(Gamez 0.9.x) do not carry over, and a group has to move across together — a
0.7 agent cannot see 0.6 peers at all. To carry a game space over, use the
**Export** command in a game's `⋯` menu on the 0.6 install to download its JSON,
then **Import** (top right of the Library panel) on the 0.7 install.
...
over every zome named in `dnas/gamez/workdir/dna.yaml` (`wasm-opt -Oz
...
The script verifies `workdir/gamez.happ` against `.happ-sha256` and
...
3. The [`release-webhapp`](.github/workflows/release-webhapp.yaml) workflow then:
   - downloads the frozen happ from `happ-v<dnaVersion>` and checks its sha256
     against `.happ-sha256`,
   - builds the UI and packs `gamez.webhapp` (no `--recursive`, so the happ
   ...
   - publishes a **prerelease** GitHub release with `gamez.webhapp` attached.
...
Add a new `versions[]` entry for `gamez` in the 0.16 curation list with the
new `version`, the release's `gamez.webhapp` `url`, and these hashes. Because
