...
#   1. `workdir/gamez.happ` from a local `npm run build:happ:release` — used when
...
if [ $# -ge 1 ]; then
  ...
  happ="$tmp/wa/gamez.happ"
else
  happ="workdir/gamez.happ"
  if [ ! -f "$happ" ]; then
...
notes="Frozen canonical gamez.happ reused by every webhapp release to keep all installs on the same network. DNA sha256: $EXPECTED_SHA. Do NOT rebuild."
...
echo "Done. release-webhapp.yaml will download gamez.happ from $HAPP_TAG."
